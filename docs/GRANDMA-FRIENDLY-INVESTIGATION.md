...
**Author:** Agorise
**Last updated:** 2026-05-19 (Part 122 cp39 — Zcash (ZEC) addition as 11th tradable asset / 8th Category-B with per-address privacy choice (transparent t1/t3 or shielded zs1/u1 via zk-SNARKs); universal no-favoritism principle adopted at cp39 and applied across all privacy-coin framing; cp33 Dogecoin (DOGE) addition as 10th tradable asset / 7th Category-B; BEP-20 network icon swap (Ken-supplied improved version); cp32 — 7 network icons swapped (Ken-supplied: ERC-20, SPL, TRC-20, Polygon, BEP-20, Base, Arbitrum), Priority #4 "TINY FOOTPRINT" introduced, lazy-loading retrofit applied across 41 below-the-fold image sites; prior cp31 — DAI tooltip + cheat-sheet row added; asset enumerations through this doc extended to all 11 tradable assets; preceding cp28 route-path drift fixes and cp27-DD2 status updates remain).
