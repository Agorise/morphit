...
**Author:** Agorise
**Last updated:** 2026-05-17 (Part 122 cp30 — USDC tooltip + cheat-sheet row added; asset enumerations through this doc extended to all 8 tradable assets; preceding cp28 route-path drift fixes and cp27-DD2 status updates remain).
...
**Status:** ✅ SHIPPED (Item 16 phase 2 / Part 90 verified; USDT tooltip added Part 121 cp3; BCH tooltip added Part 122 cp21; LTC tooltip added Part 122 cp24; USDC tooltip added Part 122 cp30). Each asset chip on `/post` carries a `<Tooltip>` next to the BTC / XMR / BLURT / USDT / USDC / BCH / LTC / DASH button — explainer copy under `post_order.form.asset_explainer.{btc,xmr,blurt,usdt,usdc,bch,ltc,dash}` translated across 10 locales. The BLURT and USDT tooltips have `faqKey="what_is_blurt"` / `faqKey="what_is_usdt"` deep-links so a curious user lands on the longer FAQ entry; BTC, XMR, BCH, LTC, and DASH chips are tooltip-only since the FAQ doesn't have dedicated entries for those.
