...
**Status:** ✅ SHIPPED (Item 16 phase 2 / Part 90 verified; USDT tooltip added Part 121 cp3; BCH tooltip added Part 122 cp21; LTC tooltip added Part 122 cp24). Each asset chip on `/post` carries a `<Tooltip>` next to the BTC / XMR / BLURT / USDT / BCH / LTC button — explainer copy under `post_order.form.asset_explainer.{btc,xmr,blurt,usdt,bch,ltc}` translated across 10 locales. The BLURT and USDT tooltips have `faqKey="what_is_blurt"` / `faqKey="what_is_usdt"` deep-links so a curious user lands on the longer FAQ entry; BTC, XMR, BCH, and LTC chips are tooltip-only since the FAQ doesn't have dedicated entries for those.
