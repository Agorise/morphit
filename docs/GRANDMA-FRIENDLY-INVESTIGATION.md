...
**Status:** ✅ SHIPPED (Item 16 phase 2 / Part 90 verified). Each asset chip on `/post` (lines 1336-1346 of `apps/web/src/routes/post/+page.svelte`) carries a `<Tooltip>` next to the BTC / XMR / BLURT button — explainer copy under `post_order.form.asset_explainer.{btc,xmr,blurt}` translated across 10 locales. The BLURT tooltip has a `faqKey="what_is_blurt"` deep-link so a curious user lands on the longer FAQ entry; BTC and XMR chips are tooltip-only since the FAQ doesn't have dedicated entries for those.
