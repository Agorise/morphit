...
**Status:** ✅ SHIPPED (Item 16 phase 2 / Part 90 verified; USDT tooltip added Part 121 cp3). Each asset chip on `/post` carries a `<Tooltip>` next to the BTC / XMR / BLURT / USDT button — explainer copy under `post_order.form.asset_explainer.{btc,xmr,blurt,usdt}` translated across 10 locales. The BLURT and USDT tooltips have `faqKey="what_is_blurt"` / `faqKey="what_is_usdt"` deep-links so a curious user lands on the longer FAQ entry; BTC and XMR chips are tooltip-only since the FAQ doesn't have dedicated entries for those.
