...
Two things the wizard does **for you, by default**: it **generates a Tor `.onion` address** in the background (so your site is reachable over Tor and shows the onion pill automatically — no waiting, no vanity grinding; if you'd already set one, it's kept), and it **hand-holds you through server hardening** — a short run of "yes" confirmations (SSH lockdown, firewall + fail2ban, automatic updates, kernel hardening, intrusion detection). The Ansible playbook (§5a) applies all of that for you; to *serve* the generated onion, point the `tor` role at the `tor-hidden-service/` directory the wizard wrote (see `OPERATIONS.md`).
