...
- **The indexer.** Reads the Blurt blockchain and remembers every Morphit-related thing that happens (orders posted, feedback left, chat messages). Serves that data to the web app. It also relays your users' *signed* actions (posting an order, sending a chat, editing a profile) to the blockchain on their behalf — a privacy win: their browsers never connect to a stranger's public Blurt server directly, so their IP addresses and exact actions stay between them and your node. (Their keys never leave their device — the indexer only forwards the already-signed message.)
...
- **ENS (`DOMAIN.eth`)** — *optional*, nothing to generate: you **register** a `.eth` name on Ethereum (ENS) and point it at your instance, usually by setting its **contenthash** to an IPFS copy of your site. Then paste the name (e.g. `morphit.eth`) into the wizard — it checks the `.eth` shape. The footer pill links to the bare name (`https://<name>.eth`), which ENS-aware browsers (Brave, MetaMask, etc.) resolve directly — Morphit deliberately does **not** route through a centralized ENS gateway (e.g. eth.limo), so the pill won't open in a vanilla browser without ENS support. (You can also set this under `morphit-ops edit → Branding & SEO`.)
