...
**Next checkpoint (deployment-gated):** stand up a real Ubuntu VM, run the full `install` → live-node arc, certify "fresh box → live, no hiccups," then optionally fold the OS-level steps into `install` itself.
