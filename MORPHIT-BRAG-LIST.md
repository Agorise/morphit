...
235. **Unit-test pass count is locked by CI.** The smoke battery (3904 scenarios) is heavy on static-analysis but doesn't run vitest. Test-rot — handlers evolving without their tests being updated — used to go undetected for months. The cp71 vitest-must-pass smoke runs `vitest --run` per workspace and asserts the pass count meets a baseline. A test-rot incident surfaces immediately as a smoke failure, so handlers and their tests stay in lock-step.
...
- **Architecture decisions**: `docs/adr/0001-*.md` through `docs/adr/0036-*.md`
...
*288 specific selling points. None of them invented. All of them shipped, documented, or honestly disclosed as backlog. If you find one that isn't accurate, open an issue at git.agorise.net/agorise/morphit and we'll either fix the claim or fix the code. Last updated 2026-05-19.*
