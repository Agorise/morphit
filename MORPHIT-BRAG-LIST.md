...
235. **Unit-test pass count is locked by CI.** The cp71 vitest-must-pass smoke runs `vitest --run` per workspace and asserts the pass count meets a baseline. Test-rot — handlers evolving without their tests being updated — used to go undetected for months because the static-analysis smoke battery doesn't run unit tests. Now a drift incident surfaces immediately as a smoke failure, so handlers and their tests stay in lock-step.
