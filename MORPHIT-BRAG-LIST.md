...
176. **Haveno/RetoSwap is Monero-only, non-PWA, not Grandma-friendly, requires you to run a node, and trust them with a required security deposit. Platform relies on unbonded, uncapped, non-random, trusted arbitrators with multisig. Arbiters and taker bots can collude for a "rug pull".** Morphit is super lightweight and trades BTC, XMR, BLURT, USDT (across four networks), USDC (across four networks), DAI (across four networks), BCH, LTC, and DASH out of the box; same multi-node federation, same orderbook, same reputation, zero third-party trust. New assets added as the community asks for them — the asset-registry pattern means days, not months.
