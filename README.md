...
Pre-launch, versioned `v1.0.0-beta.3`. The canonical public instance is **morphit.io**; community operators are welcome to launch their own nodes alongside. There are no production deployments yet — the codebase has been through an intensive multi-month pre-launch hardening campaign documented in `docs/AUDIT-2026-05.md`.
...
5. `npx morphit-ops init` to walk the setup wizard (~22 prompts; configures treasury addresses, fee targets, explorer URLs, operator tag, VAPID keys for Web Push).
6. `bash scripts/run-smokes.sh` to confirm the self-checks (~266 runners, several thousand scenarios) pass against your environment.
