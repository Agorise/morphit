...
**A federated, non-custodial, no-KYC peer-to-peer marketplace for trading fiat against Bitcoin, Monero, BLURT, and USDT.**
...
|---|---|
| `apps/web/` | SvelteKit frontend, fully prerendered per locale (10 locales × 17 indexable routes = 170 static HTML files) |
...
| `docs/` | ADRs (`docs/adr/0001-…` through `0023-…`), audit logs, operator runbooks |
...
4. `npx morphit-ops init` to walk the setup wizard (~17 prompts; configures treasury addresses, fee targets, explorer URLs, operator tag, VAPID keys for Web Push).
5. `bash scripts/run-smokes.sh` to confirm the 3,000+ self-checks pass against your environment.
...
- ADR index: `docs/adr/0001-…` through `docs/adr/0023-…`
