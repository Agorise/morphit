...
Pre-launch, versioned `v1.0.0-beta.5`. The canonical public instance is **morphit.io**; community operators are welcome to launch their own nodes alongside. There are no production deployments yet — the codebase has been through an intensive multi-month pre-launch hardening campaign documented in `docs/AUDIT-2026-05.md`.
...
6. `bash scripts/run-smokes.sh` to confirm the self-checks (~280 runners, several thousand scenarios) pass against your environment.
