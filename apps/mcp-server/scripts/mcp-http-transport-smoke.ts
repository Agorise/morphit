...
async function main(): Promise<void> {
	...
	try {
		...
		// Spawn DETACHED (own process group) and tear down with a GROUP
		// kill, so the tsx wrapper AND the node server it launches are
		...
		// exit code) | a "listening on" line (bind-OK path — the server
		// will NOT self-close, so resolve and reap it) | a timeout.
		const groupKill = (c: ChildProcess, sig: NodeJS.Signals): void => {
			try {
				if (typeof c.pid === 'number') process.kill(-c.pid, sig);
			} catch {
				/* group already gone */
			}
		};
		const spawnGuard = (host: string, port: number): Promise<{ code: number | null; err: string }> =>
			new Promise((resolve) => {
				...
				const finish = (code: number | null): void => {
					...
					clearTimeout(t);
					groupKill(c, 'SIGKILL'); // reap wrapper + server (no-op if already exited)
					resolve({ code, err });
				};
				c.stderr?.on('data', (d) => {
					err += d.toString();
					// Bind succeeded: server logged "listening on …" and will
					// not exit on its own — the guard allowed it, which is all
					// this case asserts, so resolve and reap.
					if (/listening on /.test(err)) finish(null);
				});
				c.on('close', (code) => finish(code));
				t = setTimeout(() => finish(null), 10000);
				t.unref();
			});
