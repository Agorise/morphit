...
const SCENARIOS: readonly Scenario[] = [
	...
	// docs to the corrected wording.
	{
		...
		file: 'docs/OPERATIONS.md',
		rootRelative: true,
		// The canary subsystem itself is described in §36, but
		// the chain-op variant of the canary is a ghost.
	...
	// silently regress.
	{
		name: 'cp131 LOW-010 — upgrade.ts tar extract uses --no-same-owner',
		file: 'apps/ops-cli/src/commands/upgrade.ts',
		rootRelative: true,
		mustHave: ['--no-same-owner']
	},
	{
		name: 'cp131 LOW-010 — upgrade.ts tar extract uses --no-same-permissions',
		file: 'apps/ops-cli/src/commands/upgrade.ts',
		rootRelative: true,
		mustHave: ['--no-same-permissions']
	},
	{
		name: 'cp131 LOW-010 — upgrade.ts tar extract uses --no-overwrite-dir',
		file: 'apps/ops-cli/src/commands/upgrade.ts',
		rootRelative: true,
	...
	{
		name: 'cp131 LOW-005 — main.ts no longer imports createPriceSource (single fetch loop)',
	...
	{
		name: 'cp131 LOW-005 — main.ts derives priceSource from multiAssetSources',
		file: 'apps/indexer/src/main.ts',
		rootRelative: true,
