...
const NARROW_BY_DESIGN: Array<{ pattern: string; reason: string }> = [
	...
	{
		pattern: "'btc' | 'xmr' | 'bch' | 'ltc' | 'dash' | 'doge' | 'zec' | 'arrr'",
		reason: 'urls.ts instanceTplKey — single-network explorer template targets only'
	},
	{
		pattern: "'btc' | 'xmr' | 'usdt' | 'usdc' | 'dai' | 'bch' | 'ltc' | 'dash' | 'doge' | 'zec' | 'arrr'",
...
if (findings.length === 0) {
	console.log('  ✓ all narrow unions cover the full canonical 12-asset set');
