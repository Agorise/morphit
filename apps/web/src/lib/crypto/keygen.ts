...
// bip39/secp256k1-free module. The shared [lang] layout reaches keygen on
// every page via $stores/identity (toLiveIdentity/wipeLiveIdentity) and
// $crypto/keystore (ensureSodium/Identity/KeyRole/KEY_ROLES); importing any
// keygen symbol used to drag keygen's static bip39+secp256k1 into every
...
// Re-export for the many call sites that import these from keygen.
export { KEY_ROLES, toLiveIdentity, wipeLiveIdentity };
export type { KeyRole, Keypair, FullIdentity, LiveIdentity };
export { LIVE_ROLES, JIT_ROLES } from './identity-core';
export type { Identity } from './identity-core';
...
// toLiveIdentity now lives in ./identity-core (imported + re-exported above).
...
// `Identity` (= FullIdentity) is re-exported from ./identity-core above.
