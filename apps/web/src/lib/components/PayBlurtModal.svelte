<script lang="ts">
	...
	interface Props {
		 ...
		 *  address payload's amount field upstream. */
		amount: number;
		 ...
		 *  on-chain trx_id so the parent can auto-broadcast a
		 *  morphit_funds_sent payload to close the loop. */
		onPaid: (result: { trxId: string; blockNum: number }) => void;
		/** Called on cancel / dismiss. */
		onCancel: () => void;
	}

	let { recipient, amount, memo, onPaid, onCancel }: Props = $props();
	...
	const formattedAmount = $derived(formatBlurtAmount(amount));
	...
	const canPay = $derived(
		myAccount !== null && myAccount !== recipient && amount > 0 && Number.isFinite(amount)
	);
	...
	async function confirm(): Promise<void> {
		...
		if (r.ok) {
			...
			try {
				...
				onPaid({ trxId: result.trx_id, blockNum: result.block_num });
				return;
			} catch {
				phase = {
					kind: 'error',
