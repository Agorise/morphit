...
export async function stepAltNetworks(): Promise<AltNetworkResult> {
	...
	explain(
		'Your instance AUTOMATICALLY gets a Tor .onion address —\n' +
			'generated for you at the end of setup, so Tor users can always\n' +
			'reach you and your site advertises it (the footer pill + the\n' +
			'Onion-Location auto-redirect).  Nothing to do here for Tor.\n' +
			'\n' +
			'This step is only for ADDING OTHER privacy networks, all\n' +
			'optional — you can skip and add any of them later with\n' +
			'`morphit-ops alt-address`:\n' +
			'\n' +
			'  • Lokinet   — newer, .loki address, less infrastructure\n' +
			'  • I2P       — established, .b32.i2p address\n' +
			'  • Nostr     — pubkey for cross-network discovery\n' +
			'\n' +
			'(Want a custom VANITY .onion later — one whose first letters\n' +
			'spell something? Generate it with scripts/generate-onion.sh on\n' +
			'your own machine and paste it via `morphit-ops alt-address`;\n' +
			'that replaces the basic one and is never overwritten.)'
	);

	// Tor is auto-generated by the orchestrator (basic, non-vanity) and
	// injected into the result there — never asked here.
	const tor: string | null = null;
 ...
 *      indexer's composite price source when the live
 *      upstreams are unreachable AND no value has cached
...
export async function stepHardening(bunkerWebEnabled: boolean): Promise<HardeningResult> {
	...
	// recommended-Yes confirmations (Ken's "a bunch of Yes's").  These
	// personalize the by-hand checklist; the Ansible playbook applies
	// every pillar regardless, so a "no" here never weakens the automated
	// path.
	console.log('');
