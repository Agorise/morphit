...
/** Ask whether this node runs Tor-only (no clearnet domain). Asked BEFORE the
...
export async function collectInstallInputs(
	...
	const torOnly = known.torOnly;

	// Clearnet web address — skipped entirely for a Tor-only node (its onion is
	// generated by the tor role; there's no domain to enter).
	let domain = '';
	if (!torOnly) {
