...
export function writeWizardOutput(answers: WizardAnswers, repoRoot: string): WriteResult {
	...
	let hardeningChecklistPath: string | null = null;
	let hardeningChecklistBytes = 0;
	if (answers.hardening.generateChecklist) {
		hardeningChecklistPath = join(repoRoot, 'morphit-hardening-checklist.md');
		const checklistContent = renderHardeningChecklist(answers);
		writeFileSync(hardeningChecklistPath, checklistContent, { mode: 0o644 });
		chmodSync(hardeningChecklistPath, 0o644);
		hardeningChecklistBytes = Buffer.byteLength(checklistContent, 'utf8');
	}
...
// + tls roles, ops/nginx/ configs, ops/scripts/ monitors) with the
// operator's domain + BunkerWeb-vs-nginx path filled in, and leads
// with SSH-lockout safety.  Full reference lives in OPERATIONS.md
// §34/§35/§37; this file is the personalized "do these, in this
// order, for THIS instance" runbook.
function renderHardeningChecklist(answers: WizardAnswers): string {
	const domain = deriveDomain(answers.origin);
	const bw = answers.bunkerWeb.enabled;
	const L: string[] = [];

	L.push(`# Hardening checklist — ${answers.instanceName}`);
	L.push('');
	L.push(`Personalized for this instance (public origin: ${answers.origin ?? '(not set yet)'}).`);
	L.push('Generated by `morphit-ops init`.  None of this is Morphit-specific —');
	L.push('it is the baseline every internet-facing Ubuntu server needs.');
	L.push('');
	L.push('Two ways to apply everything below:');
	L.push('');
	L.push('- **Automated:** the Ansible playbook at `ops/ansible/` applies host');
	L.push('  hardening + TLS idempotently.  Set the `enable_*` flags in');
	L.push('  `ops/ansible/group_vars/all.yml` and run it.  Lowest-error path.');
	L.push('- **By hand:** work through the steps below.  Full reference:');
	L.push('  `OPERATIONS.md` §34 (UFW + fail2ban), §35 (TLS), §37 (hardening role).');
	L.push('');
	L.push('> **SSH LOCKOUT SAFETY — read first.** Before you disable password');
	L.push('> login, add your SSH key and confirm you can log in with it from a');
	...
	L.push('      `adduser morphit && usermod -aG sudo morphit` and use it from now on.');
	L.push('- [ ] **SSH key-only login** (do these in order):');
	L.push(`      1. Copy your key up: \`ssh-copy-id you@${domain}\` (or paste it into`);
	L.push('         `~/.ssh/authorized_keys` on the server).');
	L.push(`      2. In a SECOND terminal, confirm \`ssh you@${domain}\` logs in with`);
	L.push('         the key (no password prompt).');
	L.push('      3. ONLY THEN, in `/etc/ssh/sshd_config`, set `PermitRootLogin no`');
	...
	L.push('      ```');
	L.push('      sudo ufw allow OpenSSH');
	L.push('      sudo ufw allow 80,443/tcp');
	L.push('      sudo ufw enable');
	L.push('      ```');
	L.push('      (Reference: OPERATIONS.md §34)');
	L.push('- [ ] **fail2ban** — ban brute-force SSH and repeat relay 429s:');
	L.push('      `sudo apt install fail2ban` (the sshd jail is on by default).  For the');
	L.push('      Morphit relay 429 jail and the status-alert sidecar, see OPERATIONS.md');
	L.push('      §34 and `ops/scripts/morphit-fail2ban-monitor.sh`.');
	L.push('- [ ] **Kernel sysctl hardening.**');
	L.push('      (Automated: OPERATIONS §37.3 /');
	L.push('      `ops/ansible/roles/hardening/templates/99-morphit-hardening.conf.j2`)');
	L.push('');
	L.push('## 2. TLS');
	L.push('');
	if (bw) {
	...
	L.push('');
	L.push('- [ ] fail2ban status alerts — `ops/scripts/morphit-fail2ban-monitor.sh`');
	L.push('- [ ] apt security-update alerts — `ops/scripts/morphit-apt-monitor.sh`');
	if (bw) {
		L.push('- [ ] Docker image CVE rescan (trivy) + compose health — most useful with');
		L.push('      BunkerWeb — `ops/scripts/morphit-trivy-monitor.sh`,');
		L.push('      `ops/scripts/morphit-compose-monitor.sh`');
	}
	L.push('  (Each ships a systemd timer in `ops/systemd/` and an Ansible role.)');
	L.push('');
	L.push('---');
	L.push('');
	L.push('Fastest path overall: run the Ansible playbook in `ops/ansible/`, which');
	L.push('automates sections 1–3 idempotently.  Full hardening reference:');
	L.push('OPERATIONS.md §34, §35, §37.');
	L.push('');

	return L.join('\n');
}

/** Derive a bare hostname for checklist commands from the public
 *  origin URL.  Falls back to a clear placeholder when origin was
 *  not set, so the operator sees exactly what to substitute. */
function deriveDomain(origin: string | null): string {
	if (origin === null) return '<your-domain>';
	try {
		return new URL(origin).hostname || '<your-domain>';
	} catch {
		return '<your-domain>';
	}
}
...
function renderConfig(answers: WizardAnswers): string {
	...
	lines.push('# See docs/OPERATIONS.md §45 for the full setup.');
