...
export async function unlockActiveKey(
	...
	// PREFERRED, enforced-for-production path: read the passphrase from a FILE.
	// systemd's LoadCredentialEncrypted= decrypts the credential to a
	// memory-backed (tmpfs), service-only file and the unit points
	// MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE at it. Reading from the file
	// rather than an env var keeps the secret out of the process environment
	// (/proc/<pid>/environ) entirely, and the plaintext only ever exists in
	// RAM, never on persistent disk. This is the path the shipped systemd unit
	// and OPERATIONS.md require.
	const passFile = process.env.MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE;
	if (passFile !== undefined && passFile !== '') {
		let filePass: string;
		try {
			filePass = readFileSync(passFile, 'utf8').replace(/\r?\n$/, '');
		} catch (err) {
			throw new UnlockError(
				`MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE ${JSON.stringify(passFile)} ` +
					`could not be read: ${err instanceof Error ? err.message : String(err)}`
			);
		}
		if (filePass === '') {
			throw new UnlockError(
				`MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE ${JSON.stringify(passFile)} is empty`
			);
		}
		log.info('envelope_unlock_via_credential_file');
		try {
			const wif = decryptEnvelope(cfg.relayActiveKeyEnvelope, filePass);
			return { ...cfg, relayActiveKeyWif: wif, relayActiveKeyEnvelope: undefined };
		} catch (err) {
			throw new UnlockError(
				`the passphrase in MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE did not decrypt ` +
					`the key envelope: ${err instanceof Error ? err.message : String(err)}`
			);
		}
	}

	// Fallback path: passphrase supplied directly as an env var. This works for
	// unattended startup too, but the secret then lives in plaintext in the env
	// file AND in the process environment — strictly weaker than the credential
	// file above. Supported for dev / non-systemd supervisors, but NOT the
	// production path; we log a warning so it can't be used silently.
	const envPassphrase = process.env.MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE;
	if (envPassphrase !== undefined && envPassphrase !== '') {
		log.warn('envelope_unlock_via_env_plaintext', {
			note: 'passphrase read from a plaintext env var; prefer MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE_FILE (systemd encrypted credential)'
		});
		try {
			const wif = decryptEnvelope(cfg.relayActiveKeyEnvelope, envPassphrase);
			return {
				...cfg,
			...
			throw new UnlockError(
				`MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE was set but did not decrypt the key ` +
	...
	for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
		...
		try {
			...
			if (err instanceof PassphrasePromptError) {
				...
				// no point retrying.
				throw new UnlockError(
					`passphrase prompt failed: ${err.message}. If this relay runs ` +
						`unattended (e.g. as a systemd service, with no terminal), supply ` +
						`the passphrase via the MORPHIT_RELAY_ACTIVE_KEY_PASSPHRASE ` +
