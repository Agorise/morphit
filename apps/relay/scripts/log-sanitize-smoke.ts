...
console.log('\n── relay log sanitize smoke (cp139-E-1) ─────────────────\n');

// ─── textSink: bare-string emission strips control bytes ──────

scenario('cp139-E-1: textSink strips ANSI ESC from string-without-space context value', () => {
	const r = mkRecord({
		level: 'info',
		module: 'access',
		event: 'request',
...
scenario('cp139-E-1: textSink strips C0 controls (NUL/BEL/BS) from bare value', () => {
	const r = mkRecord({
		level: 'info',
...
scenario('cp139-E-1: textSink strips DEL (0x7F) from bare value', () => {
...
scenario('cp139-E-1: textSink strips C1 (0x80-0x9F) from bare value', () => {
...
scenario('cp139-E-1: textSink with string-with-space goes JSON path (still no control bytes)', () => {
...
scenario('cp139-E-1: textSink preserves printable ASCII intact', () => {
	const r = mkRecord({
		event: 'request',
		context: {
			method: 'POST',
...
scenario('cp139-E-1: textSink preserves SGR escape (legitimate color)', () => {
...
scenario('cp139-E-1: textSink drops non-SGR ESC sequence', () => {
...
scenario('cp139-E-1: textSink sanitizes module name', () => {
...
scenario('cp139-E-1: textSink sanitizes event name', () => {
...
scenario('cp139-E-1: textSink sanitizes context key name', () => {
	const r = mkRecord({ context: { 'a\x1bb': 'val' } });
	const { stdout } = captureWrites(() => textSink(r));
	assertNotMatch(stdout, '\x1b', 'context key ESC stripped');
});

scenario('cp139-E-1: textSink sanitizes error stack trace', () => {
	const r = mkRecord({
		level: 'error',
		error: {
			name: 'TestErr',
