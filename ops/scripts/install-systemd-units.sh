...
echo
echo "Optional surfaces — deliberately NOT installed above because they run"
echo "ISOLATED (their own low-priv user + dir, so they can't read the main"
echo "install's secrets). The Ansible playbook sets both up by default:"
echo
echo "  - MCP server (AI-agent discovery): deploy to /opt/morphit-mcp, then enable:"
echo "      sudo bash ops/scripts/deploy-mcp.sh"
echo "      sudo useradd --system --no-create-home --shell /usr/sbin/nologin morphit-mcp 2>/dev/null || true"
echo "      # point it at YOUR instance (else it surfaces the default morphit.io):"
echo "      echo 'MORPHIT_MCP_INSTANCE_URL=https://YOUR-DOMAIN' | sudo tee /etc/morphit/mcp.env >/dev/null"
echo "      sudo cp ops/systemd/morphit-mcp.service /etc/systemd/system/ && sudo systemctl daemon-reload"
echo "      sudo systemctl enable --now morphit-mcp"
echo "      # toggle it on/off any time with: sudo morphit-ops mcp"
echo
echo "  - Web Push: generate a VAPID keypair ONCE (rotating it drops all"
echo "    subscriptions) into the file the relay sources, then restart the relay:"
echo "      sudo bash scripts/generate-vapid-keys.sh --bare --subject https://YOUR-DOMAIN \\"
echo "        | sudo tee /etc/morphit/relay-vapid.env >/dev/null"
echo "      sudo chmod 640 /etc/morphit/relay-vapid.env"
echo "      sudo systemctl restart morphit-relay"
echo
